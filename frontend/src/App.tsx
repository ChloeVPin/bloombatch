...
function App() {
  ...
  const addUniqueFiles = (infos: main.FileInfo[]) => {
    if (!infos || infos.length === 0) return;
    setFiles((prev) => {
      const existing = new Set(prev.map((f) => f.fullPath));
      const newFiles = infos.filter((f) => !existing.has(f.fullPath));
  ...
  useEffect(() => {
    ...
    OnFileDrop((_x, _y, paths) => {
      GetFileInfos(paths)
        .then((infos) => {
          addUniqueFiles(infos);
        })
        .catch(() => {
          // Ignore transient drag-and-drop errors.
        });
    }, true);
  ...
  const handleToggleMaximise = () => {
    ...
    window.setTimeout(() => {
      WindowIsMaximised()
        .then(setIsMaximised)
        .catch(() => {
          // Keep state unchanged; periodic sync will reconcile.
        });
    }, 30);
  };
  ...
  return (
    <div className="flex flex-col h-screen select-none bg-surface">
      <div className="h-10 flex items-center border-b border-border bg-surface-raised pl-3" style={dragStyle}>
        <div className="ml-auto flex items-stretch h-full" style={noDragStyle}>
          <button
            onClick={WindowMinimise}
            className="w-11 h-full text-text-secondary hover:bg-surface-hover active:bg-border-light transition-colors cursor-pointer flex items-center justify-center"
            aria-label="Minimize"
            title="Minimize"
          >
            <Minus className="w-4 h-4" strokeWidth={1.9} />
          </button>
          <button
          ...
          >
            {isMaximised ? <Copy className="w-3.5 h-3.5" strokeWidth={1.9} /> : <Square className="w-3.5 h-3.5" strokeWidth={1.9} />}
          </button>
          <button
            onClick={Quit}
            className="w-11 h-full text-text-secondary hover:bg-red-500 hover:text-white active:bg-red-600 transition-colors cursor-pointer flex items-center justify-center"
            aria-label="Close"
            title="Close"
          >
      ...
      <header className="flex items-center gap-2.5 px-6 py-4 border-b border-border">
        ...
        <span className="text-[15px] font-semibold tracking-tight text-text-primary">bloombatch</span>
        {page !== 'done' && stepLabels[page] && (
          <span className="text-[11px] text-text-tertiary ml-auto uppercase tracking-widest">{stepLabels[page]}</span>
        )}
      </header>

      <div className="flex flex-col flex-1 min-h-0 py-4">
        {page === 'drop' && <DropPage files={files} onFilesAdded={addUniqueFiles} onNext={() => setPage('rules')} />}
        {page === 'rules' && <RulesPage rules={rules} onRulesChange={setRules} onBack={() => setPage('drop')} onNext={() => setPage('preview')} />}
        {page === 'preview' && (
          <PreviewPage
            files={files}
            rules={rules}
        ...
        {page === 'done' && <DonePage count={doneCount} errorMessage={errorMessage} onStartOver={startOver} />}
      </div>
    </div>
  );
}
