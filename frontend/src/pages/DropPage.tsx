...
export default function DropPage({ files, onFilesAdded, onNext }: DropPageProps) {
  ...
  return (
    <div className="flex flex-col flex-1 px-6">
      {/* Dropzone — Wails uses --wails-drop-target: drop to identify valid drop targets */}
      <div
        className="dropzone flex flex-col items-center justify-center flex-1 rounded-xl border-2 border-dashed border-border bg-surface-raised hover:border-text-tertiary transition-all cursor-pointer"
      ...
      {files.length > 0 && (
        <div className="flex items-center justify-between pt-4">
          ...
          <button
            onClick={onNext}
            className="px-5 py-2 text-xs font-semibold rounded-lg text-white bg-accent hover:bg-accent-hover transition-colors cursor-pointer"
          >
            Next
          </button>
        </div>
